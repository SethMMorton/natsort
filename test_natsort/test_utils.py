...
def test__natsort_key_with_float_and_signed_splits_input_into_string_and_signed_float_with_exponent():
    ...
    assert _natsort_key('a-5.034e2', None, ns.F | ns.S) == ('a', -503.4)


def test__natsort_key_with_real_splits_input_into_string_and_signed_float_with_exponent():
    assert ns.R == ns.F | ns.S
    assert _natsort_key('a-5.034e2', None, ns.R) == ('a', -503.4)


def test__natsort_key_with_real_matches_signed_float():
    assert _natsort_key('a-5.034e2', None, ns.R) == _natsort_key('a-5.034e2', None, ns.F | ns.S)


def test__natsort_key_with_float_and_signed_and_noexp_splits_input_into_string_and_signed_float_without_exponent():
    assert ns.N == ns.NOEXP
    assert _natsort_key('a-5.034e2', None, ns.F | ns.S | ns.N) == ('a', -5.034, 'e', 2.0)


def test__natsort_key_with_float_and_unsigned_splits_input_into_string_and_unsigned_float():
    assert ns.U == ns.UNSIGNED
    assert _natsort_key('a-5.034e2', None, ns.F | ns.U) == ('a-', 503.4)
...
def test__natsort_key_with_float_and_noexp_splits_input_into_string_and_unsigned_float_without_exponent():
    assert _natsort_key('a-5.034e2', None, ns.F | ns.N) == ('a-', 5.034, 'e', 2.0)
...
def test__natsort_key_with_int_splits_input_into_string_and_unsigned_int():
    ...
    assert _natsort_key('a-5.034e2', None, ns.NOEXP) == ('a-', 5, '.', 34, 'e', 2)
    # NOEXP is ignored for integers
    assert _natsort_key('a-5.034e2', None, ns.I | ns.NOEXP) == ('a-', 5, '.', 34, 'e', 2)


def test__natsort_key_with_int_splits_and_signed_input_into_string_and_signed_int():
    assert _natsort_key('a-5.034e2', None, ns.INT | ns.SIGNED) == ('a', -5, '.', 34, 'e', 2)
    assert _natsort_key('a-5.034e2', None, ns.SIGNED) == ('a', -5, '.', 34, 'e', 2)
...
def test__natsort_key_with_tuple_input_returns_nested_tuples():
    ...
    assert _natsort_key(('a1', 'a-5.034e2'), None, ns.I) == (('a', 1), ('a-', 5, '.', 34, 'e', 2))
...
def test__natsort_key_with_input_containing_leading_numbers_returns_leading_empty_strings():
    ...
    assert _natsort_key(('15a', '6'), None, ns.I) == (('', 15, 'a'), ('', 6))
...
def test__natsort_key_with_TYPESAFE_inserts_spaces_between_numbers():
    ...
    assert _natsort_key('43h7+3', None, ns.TYPESAFE | ns.S) == ('', 43, 'h', 7, '', 3)
...
def test__natsort_key_without_string_modifiers_leaves_text_as_is():
    ...
    assert _natsort_key('Apple56', None, ns.I) == ('Apple', 56)


def test__natsort_key_with_IGNORECASE_lowercases_text():
    assert _natsort_key('Apple56', None, ns.IGNORECASE) == ('apple', 56)


def test__natsort_key_with_LOWERCASEFIRST_inverts_text_case():
    assert _natsort_key('Apple56', None, ns.LOWERCASEFIRST) == ('aPPLE', 56)


def test__natsort_key_with_GROUPLETTERS_doubles_text_with_lowercase_letter_first():
    assert _natsort_key('Apple56', None, ns.GROUPLETTERS) == ('aAppppllee', 56)
...
def test__natsort_key_with_GROUPLETTERS_and_LOWERCASEFIRST_inverts_text_first_then_doubles_letters_with_lowercase_letter_first():
    assert _natsort_key('Apple56', None, ns.G | ns.LF) == ('aapPpPlLeE', 56)
...
def test__natsort_key_with_LOCALE_transforms_floats_according_to_the_current_locale_and_strxfrms_strings():
    ...
    if use_pyicu:
        ...
        from natsort.locale_help import strxfrm
    assert _natsort_key('Apple56.5', None, ns.LOCALE | ns.F) == (strxfrm('Apple'), 56.5)
    assert _natsort_key('Apple56,5', None, ns.LOCALE | ns.F) == (strxfrm('Apple'), 56.0, strxfrm(','), 5.0)
